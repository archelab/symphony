defmodule SymphonyElixir.WorkspaceAndConfigTest do
  ...
  test "config supports per-state max concurrent agent overrides" do
    # SPEC §11.8.9: workpad defaults to enabled and forces `codex.model`
    # cross-validation. This test asserts per-state overrides and is
    # orthogonal to workpad/session-summary, so opt out explicitly.
    workflow = """
    ---
    agent:
  ...
  test "schema keeps workspace roots raw while sandbox helpers expand only for local use" do
    # SPEC §11.8.9: opt out of workpad to keep this workspace-focused test
    # orthogonal to workpad cross-validation. Session-summary is opt-in, but
