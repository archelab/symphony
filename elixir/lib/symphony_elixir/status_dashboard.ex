defmodule SymphonyElixir.StatusDashboard do
  ...
  def init(opts) do
    ...
    _ = if enabled, do: Notifications.subscribe(), else: :ok
  ...
  def handle_info(:observability_updated, %{enabled: true} = state),
    do: {:noreply, maybe_render(refresh_runtime_config(state))}

  def handle_info(:observability_updated, state), do: {:noreply, state}
  ...
  def humanize_codex_message(message), do: Telemetry.humanize_message(message)
