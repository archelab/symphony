defmodule SymphonyElixir.Observability.Projection do
  ...
  @spec issue_payload(String.t(), [map()], [map()], [map()], keyword()) ::
          {:ok, map()} | {:error, :issue_not_found}
  def issue_payload(issue_identifier, running_entries, retry_entries, completed, opts)
      when is_binary(issue_identifier) and is_list(running_entries) and is_list(retry_entries) and
             is_list(completed) and is_list(opts) do
  ...
  defp issue_payload_body(issue_identifier, running, retry, completed, opts) do
    %{
      ...
      links: links_payload(issue_identifier, running, completed),
      workspace: %{
        path: workspace_path(issue_identifier, running, retry, opts),
  ...
  defp issue_metadata(_running, [%{} = completed | _]) do
    %{
      state: "completed",
      tracker_state: Map.get(completed, :state),
      repository: repository_payload(Map.get(completed, :repository)),
      pr: pr_payload(Map.get(completed, :pr)),
      title: Map.get(completed, :title),
      kind: Map.get(completed, :kind),
      number: Map.get(completed, :number),
      branch_name: Map.get(completed, :branch_name),
      content_id: Map.get(completed, :content_id),
  ...
  defp links_payload(issue_identifier, running, completed) do
    source = running || List.first(completed) || %{}
    github_url = Map.get(source, :url) || github_url_from_identifier(issue_identifier)
    github_kind = github_link_kind(source)
  ...
  defp workspace_path(issue_identifier, running, retry, opts) do
    (running && Map.get(running, :workspace_path)) ||
      (retry && Map.get(retry, :workspace_path)) ||
      Path.join(Keyword.fetch!(opts, :workspace_root), Workspace.workspace_key(issue_identifier))
  end
