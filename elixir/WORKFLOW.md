...
- This prompt is only the Symphony-rendered first-turn prompt.
- Codex may also load repo `AGENTS.md`, global instructions, and matching
  skills outside this prompt.
- For Symphony work, if an `arche-symphony` skill is available, use it.
  Otherwise read `elixir/AGENTS.md`, `SPEC.md`, `elixir/README.md`, and
  `elixir/WORKFLOW.md` before making non-trivial changes.
- When an issue asks you to publish prompt/startup instructions, call the file
  a "publishable startup payload" unless it was generated by
  `mix symphony.prompt.dump --issue {{ issue.identifier }}`. Even generated
  dumps must state that hidden platform/system/developer instructions and
  Codex-runtime-loaded repo/skill instructions are not reproduced.
